...
protected:
		...
		std::shared_ptr<dgn::DwfStreamHandler> javaHandler;
public:
...
protected:
		std::shared_ptr<dgn::DwfStreamHandler> javaHandler;
public:
		CloseHandler(std::shared_ptr<dgn::DwfStreamHandler> javaHandler) : TK_Close_Segment(){
			this->javaHandler = javaHandler;
		}
...
protected:
		std::shared_ptr<dgn::DwfStreamHandler> javaHandler;
public:
		ShellHandler(std::shared_ptr<dgn::DwfStreamHandler> javaHandler) : TK_Shell() {
			this->javaHandler = javaHandler;
		}
		...
		TK_Status Execute(BStreamFileToolkit &parser) {
			...
			javaHandler->handleMesh(this->toDgnMesh());
			return status;
		}
};
